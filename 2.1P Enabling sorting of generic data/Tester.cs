...
namespace Vector
{
    ...
    public class Student : IComparable<Student>
    {
        ...
        {
            //sort student by Id in ASCENDING order
    ...
    public class DescendingNameDescendingIdComparer : IComparer<Student>
    {
        public int Compare(Student x, Student y)
        {
            // compare name in DESCENDING order
            int nameResult = y.Name.CompareTo(x.Name);
            // check difference between 2 names
            if (nameResult != 0)    return nameResult;  // if different -> result
            // if NOT -> compare ID
