...
namespace Heap
{
    public class Heap<K, D> where K : IComparable<K>
    {
        ...
        // left child is 2*i, right child is 2*i+1, parent is i/2
        public IHeapifyable<K, D> Delete()
        {
            if (Count == 0) 
                throw new InvalidOperationException();
            IHeapifyable<K, D> min = data[1];
            Swap(1, Count); // swap the top-most element with the last one in the heap
            data.RemoveAt(Count);   // remove the last element (the minimum) from the heap
            Count--;    
            if (Count > 0)  
                DownHeap(1);    // push the element that was moved to the root position down to its correct level
            return min;
        }

        private void DownHeap(int v)
        {
            while (2 * v <= Count)  // while the node with index v has at least one child
            {
                int j = 2 * v;  // j is the index of the left child of v
                if (j < Count && comparer.Compare(data[j].Key, data[j + 1].Key) > 0)    // if the right child of v exists 
                                                                                        // and smaller than the left child
                    j++; // update j to the index of the smallest child of v
                if (comparer.Compare(data[v].Key, data[j].Key) <= 0)    // if node v is smaller than its smallest child, 
                                                                        // then the heap property is satisfied 
                    break;
        ...
        public IHeapifyable<K, D>[] BuildHeap(K[] keys, D[] data)
        {
            ...
            // create the result array to store nodes in their ORIGINAL input order
            ...
            for (int i = 0; i < keys.Length; i++)
            {
                ...
                // store the node in the result array at the same index 'i' as the input
                result[i] = newNode;
            }

            // perform Floyd's bottom-up heap construction
            for (int i = Count / 2; i >= 1; i--)
            {
                DownHeap(i);
            }

            // return the array containing nodes in the order they were provided
            return result;
        }
        ...
        public void DecreaseKey(IHeapifyable<K, D> element, K new_key)
        {
            ...
            if (node == null || node.Position <= 0 || node.Position > Count || data[node.Position] != node)
            {
                throw new InvalidOperationException();
            }

            // check the new key 
            int comparison = comparer.Compare(new_key, node.Key);
            node.Key = new_key;

            if (comparison < 0)
            {
                // UpHeap for Min, DownHeap for Max
                UpHeap(node.Position);
                DownHeap(node.Position); 
            }
            else if (comparison > 0)
            {
                // DownHeap for Min, UpHeap for Max
                DownHeap(node.Position);
                UpHeap(node.Position);
            }
        }
