...
namespace FungusInfestation
{
    class Tester
    {
        static private void PrintMatrix(string[] matrix)
        {
            for (int i = 0; i < matrix.Length; i++) Console.WriteLine(matrix[i]);
        }
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < TestGenerator.Count(); i++)
            {
                try
                {
                    ...
                    Console.WriteLine("\nAttempting test instance {0} with the expected answer {1} for matrix ", i, result);
                    PrintMatrix(matrix);
